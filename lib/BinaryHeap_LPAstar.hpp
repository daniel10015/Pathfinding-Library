// this heap version won't work if T is type 'unsigned int'

#pragma once
#include <vector>
#include <iostream>
#include <unordered_map>
#include "../src/LPA_star.h"
...
public:
	...
	void remove(unsigned int index); // deletes element at an index
	void remove(T data); // deletes element if it exists
	...
	bool exists(T data); // checks if data exists
private:
...
template<typename T, class Compare>
void BinaryHeap<T, Compare>::remove(unsigned index)
{
	if(container.size() <= index) // out of bounds
		return;
...
template<typename T, class Compare>
void BinaryHeap<T, Compare>::remove(T data)
{
	if(exists(data))
	{
		remove(elementToIndex.at(data));
	}
}
...
template<typename T, class Compare>
bool BinaryHeap<T, Compare>::exists(T data)
{
	bool doesExist = true;
	if(elementToIndex.find(data) == elementToIndex.end())
		doesExist = false;
	return doesExist;
}
